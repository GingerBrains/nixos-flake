local function get_jdtls()
    -- Use the standard Mason installation path
    local mason_path = vim.fn.stdpath("data") .. "/mason/packages/jdtls"
    if vim.fn.isdirectory(mason_path) == 0 then
        vim.notify("JDTLS not found at " .. mason_path, vim.log.levels.ERROR)
        return nil, nil, nil
    end
...
local function get_bundles()
    -- For now, return empty bundles to avoid loading issues
    -- We can add debug and test support later once basic LSP is working
    return {}
end
...
local function setup_jdtls()
    ...
    if not launcher or not os_config or not lombok then
        vim.notify("Failed to get JDTLS paths", vim.log.levels.ERROR)
        return
    end
    ...
    local on_attach = function(client, bufnr)
        ...
        -- Standard LSP keymaps (same as in your main LSP config)
